...
logger.post('/start-pairing', async (req, res) => {
  ...
  logger.post('/edit-bot', async (req, res) => {
    const { phone, longName, shortName, prefix, owner, banner, icon, currency, link } = req.body;
    if (!phone) return res.status(400).json({ message: 'Número de teléfono no proporcionado' });
...
ñ
    ...
    global.db.data.settings[idBot] = {
      namebot: longName || global.db.data.settings[idBot].namebot,
      namebot2: shortName || global.db.data.settings[idBot].namenot2,
      ...
      prefijo: prefix || global.db.data.settings[idBot].prefijo,
